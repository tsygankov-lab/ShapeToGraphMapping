%Refactorcheccked
function plotVoronoiSkeleton_SingleRegion_ExtVsInt(records, regionID, colorMat, colorMatInt, lineParams, lineParamsInt)
if(nargin < 4 || isempty(lineParams))
    lineParams = {'LineWidth',0.5};
end
if(nargin < 5 || isempty(lineParamsInt))
    lineParamsInt = {'LineWidth',0.5};
end
...
%%Alt method - see if either edge has the correct color, then see if the origins are in the correct area
     ...
     i = regionID;
        ...
        edgeInGraphExt = edgeInGraph & records.edges.color(:) == regionID; %Only works for images
        edgeInGraphInt = edgeInGraph & ~edgeInGraphExt;
        edgeIDs = 1:size(records.edges.twinID,1);
        plotFromEdgeList(edgeIDs(edgeInGraphExt),records, colorMat,lineParams)
        plotFromEdgeList(edgeIDs(edgeInGraphInt),records, colorMatInt,lineParamsInt)
