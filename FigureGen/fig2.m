%%This used a modified version of runVoronoi along with a modified version
%%of multiLayerBorder to generate the figures for figure 2. Figure 2 shows
%%our boundary tracing strategy and the additional peturbation to corners.
...
%% Load image, add artificial border.
...
imageName = imageTest;
tic
%% Modified version of multiLayerBorder to draw the Figure2 figures (Easier to do inside said function)
[vertexCoor, EL, imageStruct] = multiLayerBorder_FIGURE2(imageName,0.8,1);
axis ij
prevTime = toc;
thisTime = prevTime;
...
function [vertexCoor, EL, imageStruct] = multiLayerBorder_FIGURE2(image, weight, plotDBG, scaleFactor)
if(nargin == 2)
    plotDBG = 0;
end
if(nargin == 1)
...
lineParams = {'lineWidth',4};
% markerParams = {'MarkerFaceColor','m','MarkerEdgeColor','m','MarkerSize',
...
if(i <= numNonHoles)
    ...
    plot(borders_T{i}(:,1),borders_T{i}(:,2),'o','MarkerFaceColor','m','MarkerEdgeColor','m','Parent',fig4,lineParams{:});

    plot([borders{i}(end,1); borders{i}(:,1)], [borders{i}(end,2); borders{i}(:,2)],'-','Color','c','Parent',fig2,lineParams{:});
        plot([borders{i}(end,1); borders{i}(:,1)], [borders{i}(end,2); borders{i}(:,2)],'o','MarkerFaceColor','m','MarkerEdgeColor','m','Parent',fig2,lineParams{:});
...
for i = 1:numObjects
    if(i <= numNonHoles)
        plot([vertArray(pixelIndOffset(i+1),1);vertArray(pixelIndOffset(i)+1:pixelIndOffset(i+1),1)],[vertArray(pixelIndOffset(i+1),2);vertArray(pixelIndOffset(i)+1:pixelIndOffset(i+1),2)],'-','Color','c','Parent',fig3,lineParams{:});
                plot([vertArray(pixelIndOffset(i+1),1);vertArray(pixelIndOffset(i)+1:pixelIndOffset(i+1),1)],[vertArray(pixelIndOffset(i+1),2);vertArray(pixelIndOffset(i)+1:pixelIndOffset(i+1),2)],'o','MarkerFaceColor','m','MarkerEdgeColor','m','Parent',fig3,lineParams{:});
