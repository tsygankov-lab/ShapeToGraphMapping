function [loadedIMAGEFiles, measurements] = loadAllIMAGEFiles(folder_name,all_para,VITRO)
...
% loadedDataStructs = cell(length(stat_filenames),1);
loadedIMAGEFiles = cell(length(stat_filenames),1);
measurements = zeros(length(stat_filenames),2);
for i = 1:length(stat_filenames)
    loadedDataStructs = load(stat_filenames{i});
    measurements(i,1) = sum(sum(loadedDataStructs.records.imageMask));
    measurements(i,2) = loadedDataStructs.dataStruct.loop.numInHoleEnclosedByObject + loadedDataStructs.dataStruct.loop.numInHoleNoEnclosure;
    loadedIMAGEFiles{i} = loadedDataStructs.records.imageMask;
end
end