function betweenness = BetweenessCentrality(V,E,loopStruct)
color = loopStruct.color;
% vertSubGraph = loopStruct.subGraphVerts;
vertFilterList = loopStruct.binaryFilt;
location = loopStruct.objectNumber;
nonLeafVerts = true(size(vertFilterList));
nonLeafVerts(loopStruct.boundaryVerts) = false;

% E = records.edges;
% V = records.verts;
%%Remove these
r2sV              = loopStruct.records2SubgraphMap_Vertices;%find(vertFilterList); %subGraph to records Mapping for vertices
s2rV              = loopStruct.subgraph2RecordsMap_Vertices;%sparse(size(vertFilterList)); %records to subgraph mapping for vertices
...
% disp('matlab coder')
% tic;
betweenness = mexBetweenessCentrality_mex(V,E,color,vertFilterList,location,full(r2sV)',s2rV,nonLeafVerts);
% disp(num2str(toc))
% disp('matlab')
% 
% tic;
...
%                     radMatrix(iter) = V.rad(vertexCollectionList(iter));%records.vorVert(vertexCollectionList(iter),3);
%                 end
%             end
%         else
