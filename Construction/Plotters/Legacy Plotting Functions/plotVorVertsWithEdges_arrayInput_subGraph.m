...
function [isNotPlotted, vorVertNorm, vorVertImplicit, vorVertNormStart, vorVertImplicitStart, vorVertVertex, vorVertVertexStart] = plotVorVertsWithEdges_arrayInput_subGraph(vertexArray, records, vertFilterFlag, isNotPlotted, vorVertNorm, vorVertImplicit, vorVertNormStart, vorVertImplicitStart, vorVertVertex, vorVertVertexStart, edgeInGraph)
...
for i = 1:vertexArray(8)
    ...
    if(isNotPlotted(vertexArray(8+i)) && edgeInGraph(vertexArray(8+i)))
        ...
        if(vertA(1) == 0 && vertA(2) == 0 || vertB(1) == 0 && vertB(2) == 0)
           disp(''); 
        end
        if(vertFilterFlag(edge(3)) && vertFilterFlag(twinEdge(3)))
