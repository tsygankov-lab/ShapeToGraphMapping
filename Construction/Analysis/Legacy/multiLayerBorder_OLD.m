function [vertexCoor, EL, numObjects, numNonHoles, objLength, colorList, pixelIndOffset, labelMat, adjMat] = multiLayerBorder_OLD(image, weight, plotDBG)
if(nargin == 2)
    plotDBG = 0;
end
if(nargin == 1)
...
for i = 1:numObjects
    ...
    subplot(1,3,1)
plot(borders_T{i}(:,1),borders_T{i}(:,2),'Color',colorSpace(i,:));
    subplot(1,3,2)
    end
...
if(i <= numNonHoles)
    ...
    if(plotDBG)
    plot(borders_T{i}(:,1),borders_T{i}(:,2),'-w');
    plot([borders{i}(end,1); borders{i}(:,1)], [borders{i}(end,2); borders{i}(:,2)],'-o','Color',[1,0,0]);
    end
...
% 	rightUpCornerCut(idexVec) = ~upDownSwitchBack(idexVec) & lsposY & posX;
    ...
    if(plotDBG)
    plot(borders_T{i}(:,1),borders_T{i}(:,2),'-w');
    plot([borders{i}(end,1); borders{i}(:,1)], [borders{i}(end,2); borders{i}(:,2)],'-x','Color',[0,1,0]);
    end
    numPixels(i) = size(borders{i},1);
end
...
for i = 1:numObjects
    if(i <= numNonHoles)
        plot([vertArray(pixelIndOffset(i+1),1);vertArray(pixelIndOffset(i)+1:pixelIndOffset(i+1),1)],[vertArray(pixelIndOffset(i+1),2);vertArray(pixelIndOffset(i)+1:pixelIndOffset(i+1),2)],'-o','Color',[1,0,0]);
    else
        plot([vertArray(pixelIndOffset(i+1),1);vertArray(pixelIndOffset(i)+1:pixelIndOffset(i+1),1)],[vertArray(pixelIndOffset(i+1),2);vertArray(pixelIndOffset(i)+1:pixelIndOffset(i+1),2)],'-x','Color',[0,1,0]);
    end
end
end
% subplot(1,2,2)
