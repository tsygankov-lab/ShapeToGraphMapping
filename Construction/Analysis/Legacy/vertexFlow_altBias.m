function [AUC_limit, AUC_flow, weightedVariability] = vertexFlow_altBias(records, loopStruct)
...
for i = 1:length(loopStruct.boundaryVerts)
    ...
    rad = radWeighted;
%     dist = cumsum([distInc]);
%     distWeighting = abs(max(dist) - dist);
    distWeighting = distInc;
%     distWeighting = 1./2.^(distInc);
    weightedVariability(i) = var(rad,distWeighting);
	AUC_flow(i) = (sum(distWeighting.*rad)/sum(distWeighting));
	for j = length(rad):-1:2
		if(rad(j) < rad(j-1))
			rad(j-1) = rad(j);
		end
	end
	AUC_limit(i) = (sum(distWeighting.*rad)/sum(distWeighting));
end