...
function [loopStruct] = extractSubGraphCycles_NoImageAssumption(vertFilterList,records,dataStruct,color,location,inObject,nonCycleFlag)
...
%     colormap('gray');
%     axis equal
%     plotSubRegion(records, color, 'c','b');
% end
E = records.edges;
V = records.verts;
%%Remove these
...
%Either it's a boundary point which must have this color... or it's a
%non-border point which must check the region
%%Messier under no image
edgeFilterList       = records.edges.color(:) == color & ((records.verts.borderRegions(records.edges.origin(:,1),1) == location | records.verts.borderRegions(records.edges.origin(:,1),2) == location)...
                                                          & (records.verts.borderRegions(records.edges.origin(:,2),1) == location | records.verts.borderRegions(records.edges.origin(:,2),2) == location));
        ...
        for i = 1:records.verts.degree(startID)
            ...
            if(E.color(twinEdge) == color && E.color(edge) ~= color)
                if(E.dist(twinEdge) > eps(max([V.pos(E.origin(twinEdge,1),1:2)])))
                    ...
                    distMatrix(1)           = 0;
                    parentEdge(r2sV(validStartPoints(iter)),1) = E.twinID(twinEdge);%(twinEdge(1));
                    %                 startEdge = edge(1);
                end
            end
        end
